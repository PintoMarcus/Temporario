...
################################
# COMPARE e REMOVE ZEREROS E VIRGULAS CASO SEJAM DIFERENETES PARA VALIDAR SE A DIFERENÇA É APENAS PRO CAUSA DAS COLUNAS DECIMAL 
    ...
    foreach ($linha in $linhas) {
        ...
        if ($hashOrigem -eq $hashDestino) {
            ...
            $conteudoDestinoSemZerosVirgulas = $conteudoDestino -replace '[0,]', ''
                ...
                $hashOrigemAjustado = (Get-FileHash -Path $origem -Algorithm MD5).Hash
                $hashDestinoAjustado = (Get-FileHash -Path $destino -Algorithm MD5).Hash

            # Verificar se os arquivos são iguais após a remoção de zeros e vírgulas
            if ($hashOrigemAjustado -eq $hashDestinoAjustado) {
...
################################
#### COMPARE USANDO HASH - SOMENTE COMPARE
# Caminho do arquivo lista.txt
$caminhoLista = "C:\temp\compare-origem\lista.txt"
